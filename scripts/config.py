...
# you can check out the default configuration in config/config.yaml
...
class option:
    def __init__(self, name: str, opt: dict) -> None:
        ...
        if not "value" in opt.keys():
            raise Exception("option \"%s\" does not have the required key: \"value\"" % self.name)

        if not "type" in opt.keys():
            raise Exception("option \"%s\" does not have the required key: \"type\"" % self.name)
        ...
        if self.type == "boolean" and isinstance(opt["value"], bool):
            ...
            raise Exception("option \"%s\" has an invalid type, use boolean, integer or string" % self.name)
